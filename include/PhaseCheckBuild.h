...
class CPhaseCheckBuild
{
	public:
	  ...
	  BOOL CnstPhaseInfo (const char *sn, LPBYTE lpData, int nDataLen,
			      int *pnRetLen);
	  BOOL CnstPhaseInfo (const char *sn1, const char *sn2, LPBYTE lpData,
			      int nDataLen, int *pnRetLen);
	   ...
	   */
	  BOOL Cnst8KBuffer (const char *sn /*IN*/, LPBYTE lpData /*OUT*/,
			     int nDataLen, const char *sn2 = NULL);
	  /*
	     Input : --> lpData
	     Output: <-- sn
	   ...
	   */
	  BOOL FindSnFrom8K (const BYTE * lpData /*IN*/, int nDataLen,
			     BYTE * sn /*OUT*/, int nSnLen);
	  //  ]]]
