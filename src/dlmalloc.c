...
#            endif
       /* USE_LOCKS && SPIN_LOCKS_AVAILABLE. */
#      endif
       /* USE_SPIN_LOCKS */
#      ifndef INSECURE
#            define INSECURE 0
...
/*
  ...
  the given space.
*/
	  void *mspace_calloc (mspace msp, size_t n_elements,
			       size_t elem_size);
...
#            pragma warning( disable : 4127 )
					/* no "conditional expression is constant" warnings */
...
#            ifndef BitScanForward
				/* Try to avoid pulling in WinNT.h */
#                  ifdef __cplusplus
extern "C"
{
#                  endif	/* __cplusplus */
...
win32munmap (void *ptr, size_t size)
{
	  ...
	  while (size)
	  {
		    if (VirtualQuery (cptr, &minfo, sizeof (minfo)) == 0)
			      return -1;
		    if (minfo.BaseAddress != cptr
			|| minfo.AllocationBase != cptr
...
static FORCEINLINE int
pthread_acquire_lock (MLOCK_T * sl)
{
	  int spins = 0;
...
#                              if defined(__linux__) || defined(__FreeBSD__) || defined(__APPLE__)
			      sched_yield ();
#                              else
			      /* no-op yield on unknown systems */
			      ;
#                              endif
			      /* __linux__ || __FreeBSD__ || __APPLE__ */
#                        endif
			      /* solaris */
		    }
	  }
...
static FORCEINLINE void
pthread_release_lock (MLOCK_T * sl)
{
	  volatile unsigned int *lp = &sl->l;
	  assert (*lp != 0);
	  ...
	  if (--sl->c == 0)
	  {
		    ...
		    int prev = 0;
		    int ret;
		    __asm__ __volatile__ ("lock; xchgl %0, %1":"=r" (ret):"m"
					  (*(lp)), "0" (prev):"memory");
	  }
}

static FORCEINLINE int
pthread_try_lock (MLOCK_T * sl)
{
	  volatile unsigned int *lp = &sl->l;
	  if (*lp != 0)
	  {
	  ...
	  else
	  {
		    ...
		    int val = 1;
		    int ret;
		    __asm__ __volatile__ ("lock; cmpxchgl %1, %2":"=a"
					  (ret):"r" (val), "m" (*(lp)),
					  "0" (cmp):"memory", "cc");
		    if (!ret)
		    {
...
init_malloc_global_mutex ()
{
	  for (;;)
	  {
			...
			(&malloc_global_mutex_status, -1, 0) == 0)
		    {
...
#      endif
					/* USE_DEV_RANDOM */
#      ifdef WIN32
					magic = (size_t) (GetTickCount () ^
							  (size_t)
							  0x55555555U);
#      else
					magic = (size_t) (time (0) ^ (size_t)
							  0x55555555U);
#      endif
...
mmap_alloc (mstate m, size_t nb)
{
	  ...
	  {			/* Check for wrap around 0 */
		    ...
		    if (mm != CMFAIL)
		    {
			      ...
			      chunk_plus_offset (p, psize)->head =
					FENCEPOST_HEAD;
			      chunk_plus_offset (p,
						 psize + SIZE_T_SIZE)->head =
					0;
...
mmap_resize (mstate m, mchunkptr oldp, size_t nb)
{
	  ...
	  else
	  {
		    ...
		    size_t newmmsize =
			      mmap_align (nb + SIX_SIZE_T_SIZES +
					  CHUNK_ALIGN_MASK);
		    ...
		    if (cp != CMFAIL)
		    {
			      ...
			      size_t psize =
					newmmsize - offset - MMAP_FOOT_PAD;
			      newp->head = psize;
			      mark_inuse_foot (m, newp, psize);
			      chunk_plus_offset (newp, psize)->head =
					FENCEPOST_HEAD;
			      ...
			      if (cp < m->least_addr)
					m->least_addr = cp;
			      if ((m->footprint +=
				   newmmsize - oldmmsize) > m->max_footprint)
					m->max_footprint = m->footprint;
...
add_segment (mstate m, char *tbase, size_t tsize, flag_t mmapped)
{
	  ...
	  char *rawsp =
		    old_end - (ssize + FOUR_SIZE_T_SIZES + CHUNK_ALIGN_MASK);
...
sys_alloc (mstate m, size_t nb)
{
	  ...
	  if (MORECORE_CONTIGUOUS && !use_noncontiguous (m))
	  {
		    ...
		    else
		    {
			      ...
			      asize = granularity_align (nb - m->topsize +
							 SYS_ALLOC_PADDING);
...
sys_trim (mstate m, size_t pad)
{
	  ...
	  if (pad < MAX_REQUEST && is_initialized (m))
	  {
		    ...
		    if (m->topsize > pad)
		    {
			      ...
			      size_t extra =
					((m->topsize - pad +
					  (unit - SIZE_T_ONE)) / unit -
					 SIZE_T_ONE) * unit;
			      msegmentptr sp =
					segment_holding (m, (char *) m->top);
...
tmalloc_large (mstate m, size_t nb)
{
	  ...
	  {			/* set t to root of next non-empty treebin */
		    binmap_t leftbits =
			      left_bits (idx2bit (idx)) & m->treemap;
		    if (leftbits != 0)
		    {
			      bindex_t i;
...
tmalloc_small (mstate m, size_t nb)
{
	  ...
	  if (RTCHECK (ok_address (m, v)))
	  {
		    ...
		    if (RTCHECK (ok_next (v, r)))
		    {
			      ...
			      {
					set_size_and_pinuse_of_inuse_chunk (m,
									    v,
									    nb);
...
ialloc (mstate m, size_t n_elements, size_t * sizes, int opts, void *chunks[])
{
	  ...
	  else
	  {
		    ...
		    marray = 0;
		    array_size =
			      request2size (n_elements * (sizeof (void *)));
	  }
	  ...
	  for (i = 0;; ++i)
	  {
		    ...
		    {		/* the final element absorbs any overallocation slop */
			      set_size_and_pinuse_of_inuse_chunk (m, p,
								  remainder_size);
			      break;
		    }
	  }
...
#      if DEBUG
	  if (marray != chunks)
	  {
		    ...
		    {
			      assert (remainder_size ==
				      request2size (sizes[i]));
		    }
		    ...
		    if (bytes <= MAX_SMALL_REQUEST)
		    {
				...
				pad_request (bytes);
			      ...
			      {	/* Remainderless fit to a smallbin. */
					...
					unlink_first_small_chunk (gm, b, p,
								  idx);
					set_inuse_and_pinuse (gm, p,
							      small_index2size
							      (idx));
					mem = chunk2mem (p);
...
#            endif
		    ...
		    if (!PREACTION (fm))
		    {
			      ...
			      if (RTCHECK
				  (ok_address (fm, p) && ok_inuse (p)))
			      {
					size_t psize = chunksize (p);
					mchunkptr next = chunk_plus_offset (p,
									    psize);
					if (!pinuse (p))
					{
						  size_t prevsize =
		    ...
		    if (bytes <= MAX_SMALL_REQUEST)
		    {
				...
				pad_request (bytes);
			      ...
			      {	/* Remainderless fit to a smallbin. */
					...
					assert (chunksize (p) ==
						small_index2size (idx));
					unlink_first_small_chunk (ms, b, p,
								  idx);
					set_inuse_and_pinuse (ms, p,
							      small_index2size
							      (idx));
					mem = chunk2mem (p);
